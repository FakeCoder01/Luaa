async function ShowOlderChatMessage(chat_id) {
  const response = await fetch(
    "/support/both/older-message?chat_id=" + chat_id,
  );
  const chat_groups = await response.json();
            ...
            messages.insertAdjacentHTML(
              ...
              `<div class="flex w-full mt-2 space-x-3 max-w-xs ml-auto justify-end">
                                <div>
                                    ...
                                    <span class="text-xs text-gray-500 leading-none">${older_message.time_ago}</span>
                                </div>
                                <div class="flex-shrink-0 h-10 w-10 rounded-full bg-red-300"></div>
                            </div>`,
            );
