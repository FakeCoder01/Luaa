...
def ConnectingFlight(
    origin, destination, seat_class, departure_date, departure_time=None
):
    try:
        origin_flights = []
        if departure_time is not None:
            print("Hit1")
            origin_flights = Flight.objects.filter(
            ...
            origin_flights = Flight.objects.filter(
                origin_airport=origin, departure_date=departure_date
            )

        destination_flights = Flight.objects.filter(destination_airport=destination)
        connecting_flights1 = Flight.objects.filter(
            origin_airport__in=origin_flights.values("origin_airport"),
            departure_time=departure_time,
        )
        connecting_flights2 = Flight.objects.filter(
            destination_airport__in=destination_flights.values("destination_airport")
        )
                    ...
                    final_list.append(
                        {
                            ...
                            "flight_duration": x["flight_duration"]
                            + y["flight_duration"],
                        }
                    )
...
def SearchFlight(request):
    try:
        ...
        if search_params.is_valid():
            ...
            connecting_flights = []
            if (
                "flight_type" in request.GET
                and request.GET["flight_type"] == "connecting"
            ):
                if request.GET.get("departure_time", "") != "":
                    departure_time = request.GET["departure_time"]
                    connecting_flights = ConnectingFlight(
                        origin, destination, seat_class, departure_date, departure_time
                    )
                else:
                    connecting_flights = ConnectingFlight(
                        origin, destination, seat_class, departure_date
                    )
                ...
                if request.GET.get("departure_time", "") != "":
                    ...
                    flights = [
                        x.search_flight_result(seat_class)
                        for x in Flight.objects.filter(
                            origin_airport=origin,
            ...
            if sort_key == "price":
                ...
                connecting_flights = sorted(
                    connecting_flights, key=lambda k: k["flight_price"]
                )

            elif sort_key == "time":
                flights = sorted(flights, key=lambda k: k["flight_duration"])
                connecting_flights = sorted(
                    connecting_flights, key=lambda k: k["flight_duration"]
                )
...
@api_view(["GET"])
def home_page_all_flights(request):
    try:
        flights = [x.search_flight_result("economy_fare") for x in Flight.objects.all()]
        payload = {
            "status_code": 200,
