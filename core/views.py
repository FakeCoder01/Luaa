...
from .models import Airport, Flight, Profile, User, Fare, Payment, Booking
        ...
        if request.method == "POST":
            ...
            if user:
                ...
                if Profile.objects.filter(
                    user=request.user, profile_active=True
                ).exists():
                    ...
                    return redirect("/profile?perform=new")
            else:
                messages.error(request, "Invalid credentials")
                return redirect("/login?msg=invalid credentials")
        return render(
            request,
            "booking/authentication/login.html",
            {"page_title": "Login to Luua"},
        )
    except Exception as err:
        logger.error(err)
    ...
    try:
        if request.method == "POST":
            ...
            if user:
                ...
                return redirect("/profile?res=account created successfully&perform=new")
        ...
        return render(
            request,
            "booking/authentication/sign-up.html",
            {"page_title": "Sign up to Luua"},
        )
    except Exception as err:
        logger.error(err)
...
def manage_profile(request):
    try:
        ...
        if request.method == "POST":
            ...
            if (
                profile_active == True
                and "full_name" in request.POST
                and request.POST["full_name"] != ""
            ):
            ...
            return redirect("/profile?res=Profile saved successfully")
        context = {
...
def flight_deatils(request, flight_id):
    try:
        if Flight.objects.filter(uid=flight_id).exists():
            ...
            if (
                request.method == "POST"
                and "seat_class" in request.POST
                and request.POST["seat_class"] != ""
            ):
                response = HttpResponseRedirect(
                    f"/flight/{flight_id}/book/?seat_class={request.POST['seat_class']}"
                )
                request.session["seat_class"] = request.POST["seat_class"]
                return response
            context = {
                "page_title": flight.origin_airport.iata_code
                + " to "
        ...
        if Flight.objects.filter(uid=flight_id).exists():
            ...
            if request.method == "POST":
                ...
                if flight_booking.is_valid():
                    if (
                        "flight_id" not in request.POST
                        or "seat_type" not in request.POST
                        or "fare_amount" not in request.POST
                    ):
                        return redirect(f"/flight/{flight_id}/?flight-params")

                    if (
                        request.POST["flight_id"] != flight_id
                        or float(request.POST["fare_amount"]) != fare_amount
                    ):
                    ...
                    if flight.seats_left().get(seat_cat.lower(), 0) == 0:
                        messages.error(request, "Flight Full !!")
                        messages.error(
                            request, f"All {seat_cat} category seats are already booked"
                        )
                    ...
                    try:
                        ...
                        pass
                    return redirect(
                        f"/flight/{flight_id}/payment/{new_flight_booking.uid}/pay"
                    )
...
def invoice_payment(request, payment_id):
    try:
        if Payment.objects.filter(
            payment_id=payment_id, profile=request.user.userprofile
        ).exists():
...
def book_flight(request, flight_id, booking_id):
    try:
        if (
            Flight.objects.filter(uid=flight_id).exists()
    ...
    try:
        context = {
            ...
            "bookings": Booking.objects.filter(
                profile=request.user.userprofile, confirmed=True
            ),
        }
...
def booking_details(request, booking_id):
    try:
        if Booking.objects.filter(
            uid=booking_id, profile=request.user.userprofile
        ).exists():
...
def privacy_security(request):
    context = {"page_title": "Privacy & Security | Luua", "tab_id": "privacy-security"}
    return render(request, "booking/account/privacy-security.html", context)


def homepage(request):
    try:
        context = {"flights": Flight.objects.all()[:16]}
    ...
    try:
        if request.method == "POST":
            try:
                ...
                if (
                    result is not None
                    and Payment.objects.filter(payment_id=razorpay_order_id).exists()
                ):
                    ...
                    try:
                        ...
                        return render(
                            request,
                            "booking/pay/paymentsuccess.html",
                            {"payment": payment, "page_title": "Success"},
                        )
                ...
                return render(
                    request, "booking/pay/paymentfail.html", {"page_title": "Failure"}
                )
            except Exception as err:
                logger.info(err)
